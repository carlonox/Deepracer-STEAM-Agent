...
## Anexo: Sistema RAG (Generación Aumentada por Recuperación)
 ...
 - Dependencias listadas en `apps/rag/requirements.txt`
 ...
 3. Editar `apps/rag/.env` si es necesario para cambiar el modelo o URL de Ollama.
 ...
 ```bash
 python apps/rag/src/indexador.py
 ```

 Esto procesará todos los archivos `.md` de `apps/rag/knowledge/` y creará el índice en `apps/rag/faiss_index/` (ignorado por Git). La primera ejecución tarda 1-3 minutos (descarga del modelo de embeddings).
...
| El índice no existe | No se ejecutó el indexador | Ejecutar `python apps/rag/src/indexador.py` primero |
