...
def cargar_documentos(ruta: str) -> list[dict]:
    """
    Lee todos los .md de knowledge/ y retorna lista de dicts con contenido y metadata.
...
def main():
    ...
    # Rutas resueltas desde el archivo, no desde el CWD
    ruta_rag = Path(__file__).resolve().parent.parent
    ruta_conocimiento = ruta_rag / "knowledge"
    ruta_indice = ruta_rag / "faiss_index"
    
    logger.info("Iniciando indexación RAG del aula STEAM...")
    
    # Verificar que existen archivos .md
    rutas_md = glob.glob(str(ruta_conocimiento / "*.md"))
    if not rutas_md:
        logger.error(f"No se encontraron archivos .md en {ruta_conocimiento}")
        return
    
    # Cargar documentos
    logger.info("Cargando documentos...")
    documentos = cargar_documentos(str(ruta_conocimiento))
