...
// ⚠️ PRUEBA DE HARDWARE: esta herramienta MUEVE EL VEHÍCULO.
// Requiere autorización explícita, zona despejada y operador presente.
// No ejecutar como prueba automática.
...
const log = {
  i: (m) => console.log(`${COLORS.cyan}[i] ${m}${COLORS.reset}`),
  s: (m) => console.log(`${COLORS.green}[OK] ${m}${COLORS.reset}`),
  w: (m) => console.log(`${COLORS.yellow}[!] ${m}${COLORS.reset}`),
  e: (m) => console.error(`${COLORS.red}[X] ${m}${COLORS.reset}`),
};
