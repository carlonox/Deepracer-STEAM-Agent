...
| 2026-09-05 | Purga de secretos del historial (`filter-repo`, 3 secretos → `***REMOVED***`, gitleaks 0) + gobernanza: `main` protegida (PR + 1 review + CI verde), Conventional Commits obligatorio, sin push directo ni `--force`. | Historial con credenciales + equipo multi-persona sin reglas. | Todo el historial (SHAs nuevos), `AGENTS.md`, `SECURITY.md`, `.github/workflows/`, skill `conventional-commits-deepracer`. | Requiere `fetch` + `reset --hard` o re-clonar; rotación pendiente en robot. |
| 2026-09-05 | Componente `apps/simulator/`: mock fiel de la API (login CSRF, watchdog 200ms, dead zone, MJPEG) con 19 tests. | Desarrollar sin robot físico. | `apps/simulator/`, índice `apps/README.md`. | Sin hardware; certs autofirmados ignorados. |
| 2026-09-08 | Split skill `deepracer-control` (1716 líneas) en 4 (`motor-control`, `calibration`, `vision`, `troubleshooting`) + router v3.0.0; ESP32 como apéndice legacy. | Skill monolítica inmantenible (patrón Thalor). | `hermes/skills/robotics/deepracer-*/`, `hermes/skills/README.md`, Fase 6 del plan. | Auditoría: 0 líneas perdidas; `references/`, `scripts/`, `templates/` quietos. |
| 2026-09-08 | `hermes/soul/soul.md` reescrito (mascota del aula, backend-only, RAG); legacy archivado en `docs/archive/SOUL-legacy-20260908.md`. | SOUL desactualizado y duplicado (Fase 2). | `hermes/soul/soul.md`, `docs/archive/`, índices. | Memoria (Fase 3) pendiente. |
| 2026-09-08 | Revisor propio de PRs: workflow `pr-reviewer.yml` (PR-Agent + OpenRouter) + `.pr_agent.toml` + doc. | Complementar a CodeRabbit en reglas del robot, $0. | `.github/workflows/pr-reviewer.yml`, `.pr_agent.toml`, `docs/development/pr-reviewer.md`. | Requiere secreto `OPENROUTER_API_KEY`; `main` protegida igual. |
